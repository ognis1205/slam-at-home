...
public protocol WebRTC: AnyObject {
  /// WebRTC offer call.
...
public protocol WebRTCDelegate: AnyObject {
  ...
  func webRTC(_ client: WebRTC, didDiscoverLocalCandidate candidate: RTCIceCandidate)

  /// Called when connection state has changed.
  func webRTC(_ client: WebRTC, didChangeConnectionState state: RTCIceConnectionState)

  /// Called when new data has recieved.
  func webRTC(_ client: WebRTC, didReceiveData data: Data)
}
