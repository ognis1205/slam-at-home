...
public class WebRTC: NSObject, WebRTCProtocol {
  ...
  public required init(iceServers: [String]) {
    ...
    self.connection = WebRTC.factory.peerConnection(
      with: config,
  ...
  private func createAudioTrack() -> RTCAudioTrack {
    ...
    let source = WebRTC.factory.audioSource(with: constrains)
    let audioTrack = WebRTC.factory.audioTrack(with: source, trackId: self.audioTrackId)
    return audioTrack
  }

  /// Creates video track.
  private func createVideoTrack() -> RTCVideoTrack {
    self.videoSource = WebRTC.factory.videoSource()
    #if TARGET_OS_SIMULATOR
      self.videoCapturer = RTCFileVideoCapturer(delegate: videoSource)
    #else
      self.videoCapturer = RTCCameraVideoCapturer(delegate: self)
    #endif
    let videoTrack = WebRTC.factory.videoTrack(with: videoSource, trackId: self.videoTrackId)
    return videoTrack
  }
