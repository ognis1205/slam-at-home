...
public extension Message: Codable {
  /// Initializer.
...
public protocol SignalingDelegate: AnyObject {
  ...
  func didConnect(_ signal: SignalingProtocol)

  /// Called when connection has destroyed.
  func didDisconnect(_ signal: SignalingProtocol)

  /// Called when SDP request are sent.
  func signal(_ signal: SignalingProtocol, didReceiveRemoteSdp sdp: RTCSessionDescription)

  /// Called when ICE request are sent.
  func signal(_ signal: SignalingProtocol, didReceiveCandidate candidate: RTCIceCandidate)
}
